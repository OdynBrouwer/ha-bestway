...
class GizwitsWebSocket(BaseWebSocketClient):
    ...
    async def connect(self) -> None:
        """Connect to the Gizwits WebSocket API, log in, and keep retrying
        until disconnect() is called.

        A failed attempt is followed by the backoff chain rather than
        surfacing to the caller, so this can be awaited once and left to
        recover on its own.
        """
    ...
    async def _connect_once(self) -> None:
        """One connect + login attempt; schedules the backoff chain on failure."""
