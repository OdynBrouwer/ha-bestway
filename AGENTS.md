...
A fourth backend must also classify its setup failures the way the existing three do: a cloud that is slow or unreachable raises `ConfigEntryNotReady`, which Home Assistant retries with backoff, and only rejected credentials raise `ConfigEntryAuthFailed`. Letting any other exception escape `async_setup_entry` puts the entry in `SETUP_ERROR`, which is never retried — every entity stays unavailable until someone reloads the entry by hand.
